...
static int or3_command(champion_t *champion, int paramthree)
{
...
static int or2_command(champion_t *champion, int *paramone, int *paramtwo,
    pc_t *op)
{
...
int or_command(global_t *global, champion_t *champion, pc_t *op)
{
    ...
    if (paramthree < 1 || paramthree > REG_NUMBER)
        return -1;
    if (or2_command(champion, &paramone, &paramtwo, op) == -1)
        return -1;
    ...
    return or3_command(champion, paramthree);
}
