...
static int lei(global_t *global, champion_t *champion,
...
/**
 * @brief           Loads a value from memory using indirect addressing
...
int ldi_command(global_t *global, champion_t *champion, pc_t *op)
{
    ...
    sum = one + paramtwo;
    return lei(global, champion, (pc_copy + sum % IDX_MOD) % MEM_SIZE, three);
}
