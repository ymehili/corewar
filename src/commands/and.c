...
static int and3_command(champion_t *champion, int paramthree)
{
...
static int and2_command(champion_t *champion, int *paramone, int *paramtwo,
    pc_t *op)
{
...
int and_command(global_t *global, champion_t *champion, pc_t *op)
{
    ...
    if (and2_command(champion, &paramone, &paramtwo, op) == -1)
        return -1;
    ...
    return and3_command(champion, paramthree);
}
